...
class PlanningGraph():
    ...
    def add_action_level(self, level: int) -> None:
        ...
        """
        # planning graph as described in the Russell-Norvig text
        # 1. determine what actions to add and create those PgNode_a objects
        # 2. connect the nodes to the previous S literal level
        # for example:
        #  a. the A0 level will iterate through all possible actions for the problem
        #  b. and add a PgNode_a to a_levels[0] set iff all prerequisite literals
        #     for the action hold in S0.
        #     This can be accomplished by testing to see if a proposed PgNode_a has prenodes
        #     that are a subset of the previous S level.
        #  c. Once an action node is added, it MUST be connected to the S node instances
        #     in the appropriate s_level set.

        # PgNode: Base class for planning graph nodes.
        # includes instance sets common to both types of nodes used in a planning graph:
        # S-type A planning graph node representing a state (literal fluent) from a planning problem
        # A-type (action) Planning Graph node - inherited from PgNode
        ...
        # records conflicts between actions that would prevent them from occurring together
        ...
        for action in self.all_actions:
            # b. and add a PgNode_a to a_levels[0] set if all prerequisite literals
            # for the action hold in S0.
    ...
    def add_literal_level(self, level: int) -> None:
        ...
        # 2. connect the nodes
        # for example:
        #   a. every A node in the previous level has a list of S nodes in effnodes that
        #       represent the effect produced by the action.
        #   b. These literals will all be part of the new S level.
        ...
        #      and connect all of the new S nodes as children of all the A nodes that could produce
        ...
        for action_node in self.a_levels[level - 1]:
            ...
            # has a list of S nodes in effnodes that represent the effect produced by the action.
            for state_node in action_node.effnodes:

                # connect all of the new S nodes as children of all the A nodes that could produce
                # them, and likewise add the A nodes to the parent sets of the S nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # inconsistent effects - effect clobbers effect

        # Inconsistent Effects:  effect of one action is negation of effect of another
        # see https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/
        # 6-825-techniques-in-artificial-intelligence-sma-5504-fall-2002/
        # lecture-notes/Lecture12FinalPart1.pdf

        effects_add_1 = set(node_a1.action.effect_add)
        effects_rem_1 = set(node_a1.action.effect_rem)

        effects_add_2 = set(node_a2.action.effect_add)
        effects_rem_2 = set(node_a2.action.effect_rem)

        # returning True if one action negates an effect of the other
        if effects_add_1.intersection(effects_rem_2):
            return True

        if effects_add_2.intersection(effects_rem_1):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # interference - effect clobbers precondition

        effects_add_1 = set(node_a1.action.effect_add)
        effects_rem_1 = set(node_a1.action.effect_rem)
        precond_pos_1 = set(node_a1.action.precond_pos)
        precond_neg_1 = set(node_a1.action.precond_neg)

        effects_add_2 = set(node_a2.action.effect_add)
        effects_rem_2 = set(node_a2.action.effect_rem)
        precond_neg_2 = set(node_a2.action.precond_neg)
        precond_pos_2 = set(node_a2.action.precond_pos)

        if effects_add_1.intersection(precond_neg_2):
            return True

        if effects_add_2.intersection(precond_neg_1):
            return True

        if effects_rem_1.intersection(precond_pos_2):
            return True

        if effects_rem_2.intersection(precond_pos_1):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # competing needs - preconditions are mutex (one needs P, one needs ~P)
        ...
        for parent_1, parent_2 in [(parent_1, parent_2) for parent_1 in node_a1.parents
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # the class tracks whether the literal is positive or negative.
        if node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos:
            # in case equal with different polarity
            negation = True
        ...
        for action_s1 in node_s1.parents:
            for action_s2 in node_s2.parents:
                ...
                if not action_s1.is_mutex(action_s2):
                    # there is an action that could achieve the two literals at the same time
                    return False

        return True

    def __h_levelcost(self, goal) -> int:
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            level_sum += self.__h_levelcost(goal)
